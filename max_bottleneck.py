...
def find_bottleneck(graph, path):
    ...
    for i in graph['graph']:
        if (i[0] == curr or i[1] == curr) and (i[0] == graph['end'] or i[1] == graph['end']):
            bottleneck['weight'] = i[2]
    while path:
        curr = path.pop()   
        for i in graph['graph']:
            if (i[0] == curr or i[1] == curr) and (i[0] == bottleneck['path'][-1] or i[1] == bottleneck['path'][-1]):
                if bottleneck['weight'] > i[2]:
                    bottleneck['weight'] = i[2]
        bottleneck['path'].append(curr)
    for i in graph['graph']:
        if (i[0] == bottleneck['path'][-1] or i[1] == bottleneck['path'][-1]) and (i[0] == graph['start'] or i[1] == graph['start']):
            if bottleneck['weight'] > i[2]:
                bottleneck['weight'] = i[2]
...
def solve_max_bottleneck(graph):
    ...
    for i in sorted_edges:
        ...
        if i[1] not in prio_queue:            
            prio_queue.append(i[1])
    dist = {}
    prev = {}
    ...
    while prio_queue:
        ...
        for i in graph['graph']:
            if i[0] == u and i[1] in prio_queue:
                tmp = dist[u] + 1
                if tmp <= dist[i[1]] and i[2] > bottleneck['weight']:
                    dist[i[1]] = tmp
                    prev[i[1]] = u
                    if i[0] in bottleneck['path'] or i[1] in bottleneck['path']:
                        bottleneck = update_bottleneck(graph, prev)
            elif i[1] == u and i[0] in prio_queue:
                tmp = dist[i[0]] + 1
                if tmp <= dist[u]  and i[2] > bottleneck['weight']:
                    dist[u] = tmp
                    prev[u] = i[0]
                    if i[0] in bottleneck['path'] or i[1] in bottleneck['path']:
                        bottleneck = update_bottleneck(graph, prev)
